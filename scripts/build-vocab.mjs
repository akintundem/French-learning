...
function parse() {
  ...
  for (const raw of lines) {
    ...
    const h2 = line.match(/^##\s+([ABCD])(\d+)\.\s+(.+)$/);
    if (h2) {
    ...
    // Other headings close the open table but keep the module, so D1's four
    // sub-tables all collect into one module.
    if (line.startsWith("#")) {
      header = null;
      continue;
    }
...
function add(mod, fr, en, opts = {}) {
  ...
  if (fr.split(",").length > 2 || en.split(",").length > 2) return;
